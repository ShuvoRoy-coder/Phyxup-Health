...
<template>
    ...
    <section ref="sectionRef" class="relative min-h-[300vh] bg-sectionBg1 flex flex-col justify-between">
        <!-- Inner content is pinned -->
        <div class="sticky top-0 min-h-screen pb-10 flex items-center z-10">
            <motion.div :initial="{ opacity: 0, y: 50 }"
                ...
                <!-- Heading -->
                <SectionTitle extra-class="text-center pb-8 sm:pb-12 xl:pb-20">
                ...
                <motion.div :initial="{ opacity: 0, y: 50 }"
                    ...
                    class="flex flex-col lg:flex-row items-center gap-6 justify-between">
                    <!-- Left Text -->
                    <div class="w-full lg:w-[28%] space-y-4">
                    ...
                    <!-- Right Video -->
                    <div class="relative w-full lg:w-[72%] @container">
                        <div class="rounded-2xl shadow-xl overflow-hidden
                        bg-[url('/images/videoplayerBg.webp')] bg-cover bg-no-repeat bg-center">
                            <div class="w-full sm:px-[18px] sm:py-[20px] md:px-[30px] md:py-[32px]">
                                <div class="rounded-2xl py-4 px-4 md:py-4 md:px-[18px] overflow-hidden sm:bg-white/20">

                                    <!-- Video transition -->
                                    <motion.div :key="videos[activeVideo].id" :initial="{ opacity: 0, scale: 0.95 }"
                                        :animate="{ opacity: 1, scale: 1 }" :exit="{ opacity: 0, scale: 0.95 }"
                                        :transition="{ duration: 0.6, ease: 'easeOut' }"
                                        class="rounded-2xl p-3 md:p-4 bg-white aspect-video">
                                        <video :src="videos[activeVideo].src" loop muted playsinline controls autoplay
                ...
                <motion.div :initial="{ opacity: 0, y: 50 }"
                    :inView="{ opacity: 1, y: 0, transition: { duration: 0.6, ease: 'easeOut' } }"
                    :viewport="{ once: false, amount: 0.3 }"
                    class="@container w-full lg:w-[70%] ml-auto pt-[25px] md:pt-[30px] xl:pt-[56px]">
                    <div class="flex flex-col md:flex-row items-center gap-[10px] md:gap-[2.3cqw]">
                        <SmartMonitoringButton v-for="(video, index) in videos" :key="video.id"
                            :active="activeVideo === index" @click="activeVideo = index">
                            <span>{{ video.label }}</span>


                            <svg v-if="index === 0" class="size-[28px] md:size-[3cqw]" viewBox="0 0 27 28" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path d="M14.7393 10L18.7393 14L14.7393 18M8.07302 10L12.073 14L8.07302 18"
                            ...
                            <svg v-if="index === 1" class="size-[28px] md:size-[3cqw]" viewBox="0 0 29 32" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                            ...
                            <svg v-if="index === 2" class="size-[28px] md:size-[3cqw]" viewBox="0 0 27 28" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
        ...
        <svg class="max-w-[1663px] mx-auto rotate-180 translate-y-[calc(100%-2px)]" viewBox="0 0 1663 127" fill="none"
            xmlns="http://www.w3.org/2000/svg">
